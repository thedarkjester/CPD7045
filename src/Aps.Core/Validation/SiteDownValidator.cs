...
{
    public class SiteDownValidator : IValidator
    {
        ...
        public void Validate(IList<InterpretedScrapeSessionDataPair> interpretedScrapeSessionDataPair, Guid customerId, Guid billingCompanyId)
        {
            ...
            if (errors.Count() > 0)
            {
                if (interpretedScrapeSessionDataPair.Where(a => a.keyValuePair.Value.Equals(Convert.ToString((int)ScrapingErrorResponseCodes.BillingCompanySiteDown))).Count() > 0)
                {
                    DataScraperException exception = new DataScraperException(Integration.EnumTypes.ScrapingErrorResponseCodes.BillingCompanySiteDown);
                    throw exception;
                }
            }
        }
